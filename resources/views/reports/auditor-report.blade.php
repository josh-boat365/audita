<x-base-layout>
    ...
    @push('scripts')
            ...
            $(document).ready(function() {
                ...
                function enhanceReportsWithBranchNames() {
                    allReports = allReports.map(report => ({
                        ...report,
                        branchName: getBranchName(report.exceptionBatchId)
                    }));
                }
                ...
                function generateReportTitle(filters) {
                    ...
                    return `BESTPOINT SAVINGS AND LOANS LIMITED INTERNAL AUDIT DEPARTMENT EXCEPTION SHEET FOR ${filters.branch.toUpperCase()} BRANCH DURING THE ${auditPeriod} AUDIT`;
                }
                ...
                function generateExecutiveSummary(data, filters) {
                    ...
                    // Build summary
                    let summary =
                        `During the ${auditPeriod} internal audit exercise at ${filters.branch} Branch, our audit team conducted a comprehensive review of operational processes, controls, and compliance measures. `;

                    summary +=
                        `This examination resulted in the identification of ${totalExceptions} exception${totalExceptions !== 1 ? 's' : ''} across various operational areas that require management attention and corrective action. `;

                    if (uniqueDepartments.length > 0) {
                        summary +=
                            `The exceptions were identified across ${uniqueDepartments.length} department${uniqueDepartments.length !== 1 ? 's' : ''}, `;
                    ...
                    summary +=
                        `Management has demonstrated commitment to addressing audit findings with ${resolvedCount} exception${resolvedCount !== 1 ? 's' : ''} already resolved`;

                    if (pendingCount > 0) {
                        summary +=
                            `, while ${pendingCount} exception${pendingCount !== 1 ? 's remain' : ' remains'} pending resolution`;
                    }

                    summary += `, achieving a ${resolutionRate}% resolution rate. `;
        ...
        <div class="exception-report mb-5 p-4" style="border: 1px solid #e9ecef; border-radius: 8px; background: #fafafa;">
            ...
            <div class="exception-analysis mb-3" style="line-height: 1.8;">
                ${generateExceptionAnalysis(primaryException, exceptions)}
            </div>

            <!-- Management Response Request -->
            <div class="management-request mb-3" style="font-style: italic; color: #666;">
                <em>Kindly explain why the case.</em>
            </div>

            <!-- Management Response -->
            <div class="management-response" style="background: #e8f4f8; padding: 15px; border-radius: 6px; border-left: 4px solid #17a2b8;">
                ...
                function generateExceptionDescription(primaryException, allExceptions) {
                    ...
                    if (allExceptions.length > 1) {
                        ...
                        description +=
                            `<br><br><strong>Scope:</strong> This exception was identified in ${allExceptions.length} instances`;

                        if (departments.length > 1) {
                            description += ` across multiple departments (${departments.join(', ')})`;
                        } else if (departments.length === 1) {
                            description += ` within the ${departments[0]} department`;
                        }
                ...
                function generateExceptionAnalysis(primaryException, allExceptions) {
                    ...
                    // Risk assessment
                    const riskRates = [...new Set(allExceptions.map(e => e.riskRate))].filter(r => r);
                    if (riskRates.length > 0) {
                        const highRiskCount = allExceptions.filter(e => e.riskRate === 'High').length;
                        const mediumRiskCount = allExceptions.filter(e => e.riskRate === 'Medium').length;
                        const lowRiskCount = allExceptions.filter(e => e.riskRate === 'Low').length;

                        if (highRiskCount > 0) {
                            analysis +=
                                `This represents a high-risk exposure that requires immediate management attention and corrective action. `;
                        } else if (mediumRiskCount > 0) {
                            analysis +=
                                `This constitutes a medium-risk issue that needs prompt management intervention to prevent potential operational impact. `;
                        } else if (lowRiskCount > 0) {
                            analysis +=
                                `While classified as low-risk, this exception still requires management attention to maintain operational excellence. `;
                        }
                    }
                    ...
                    if (departments.includes('Operations') || processTypes.includes('Customer Service')) {
                        analysis +=
                            `The operational nature of this exception could impact customer service delivery and branch efficiency. `;
                    }

                    if (departments.includes('Credit') || processTypes.includes('Loan Processing')) {
                        analysis +=
                            `This exception in credit operations could affect loan portfolio quality and regulatory compliance. `;
                    }

                    if (departments.includes('Treasury') || processTypes.includes('Cash Management')) {
                        analysis +=
                            `The treasury-related nature of this exception poses risks to cash management and financial controls. `;
                    }
                    ...
                    if (unresolvedCount > 0) {
                        analysis +=
                            `With ${unresolvedCount} instance${unresolvedCount !== 1 ? 's' : ''} still pending resolution, immediate action is required to mitigate ongoing operational risks.`;
                    } else {
                        analysis +=
                            `All instances of this exception have been resolved, demonstrating effective management response to audit findings.`;
                    }

                    return analysis ||
                        'This exception requires management review and appropriate corrective measures to ensure operational compliance and effectiveness.';
                }
        ...
        <style>
            /* Updated CSS for structured exception reports */
            .exception-report {
                margin-bottom: 2rem;
                padding: 1.5rem;
